import { last, mapKeys, range } from 'lodash';
import { LifeOpcode as LifeOpcodeLBA1 } from './data/lba1/life';
import { MoveOpcode as MoveOpcodeLBA1 } from './data/lba1/move';
import { LifeOpcode as LifeOpcodeLBA2 } from './data/lba2/life';
import { MoveOpcode as MoveOpcodeLBA2 } from './data/lba2/move';
import { ConditionOpcode as ConditionOpcodeLBA1 } from './data/lba1/condition';
import { ConditionOpcode as ConditionOpcodeLBA2 } from './data/lba2/condition';
...
export function parseScript(actor: number, type: 'life' | 'move', script: DataView) {
    ...
    while (state.offset < script.byteLength) {
        ...
        try {
            const command = parseCommand(state, script, op, type)
            command.raw = range(beginOffset, state.offset).map((value) => script.getUint8(value));
            state.commands.push(command);
        } catch (e) {
            // tslint:disable-next-line:no-console
            console.log(beginOffset, state.offset);
            console.log(range(beginOffset, state.offset));
            console.log(range(beginOffset, state.offset).map((value) => script.getUint8(value)));
